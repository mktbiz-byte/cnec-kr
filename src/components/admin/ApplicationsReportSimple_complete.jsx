...
const ApplicationsReportSimple = () => {
  ...
  const { campaignId } = useParams() // URL에서 특정 캠페인 ID를 받을 수 있음
  ...
  const texts = {
    ko: {
      ...
      virtualSelectSuccess: '가상 선택되었습니다.',
      virtualCancelSuccess: '가상 선택이 취소되었습니다.',
      finalConfirmSuccess: '최종 확정되었습니다.',
      search: '검색',
      refresh: '새로고침',
      name: '이름',
      email: '이메일',
      ...
      campaignReport: '캠페인 보고서'
    },
    ja: {
      title: '지원 관리',
      subtitle: 'すべてのキャンペーン지원を管理します',
      totalApplications: '総지원数',
      pendingApplications: '대기 중',
      approvedApplications: '승인됨',
      rejectedApplications: '거부됨済み',
      virtualSelected: '仮選択',
      ...
      noApplications: '지원がありません。',
      applicationDetails: '지원상세情報',
      ...
      submittedAt: '지원日',
      updatedAt: '업데이트日',
      statusUpdated: 'ステータスが업데이트されました。',
      statusUpdateFailed: 'ステータスの업데이트に실패했습니다。',
      virtualSelectSuccess: '仮選択されました。',
      virtualCancelSuccess: '仮選択が取り消されました。',
      finalConfirmSuccess: '最終確定されました。',
      search: '검색',
      refresh: '업데이트',
      name: '名前',
      email: 'メール',
  ...
  const handleVirtualSelection = async (applicationId, isSelected) => {
    try {
      ...
      console.log('가상 선택 업데이트 시작:', applicationId, isSelected)

      // 타임아웃 설정
      const timeout = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('가상 선택 업데이트 타임아웃')), 10000)
      )
      ...
      console.log('가상 선택 업데이트 완료')
      
      setSuccess(isSelected ? t.virtualSelectSuccess : t.virtualCancelSuccess)
      
      // 로컬 상태 업데이트
      setApplications(prev => prev.map(app => 
        app.id === applicationId 
          ? { ...app, virtual_selected: isSelected, virtual_selected_at: isSelected ? new Date().toISOString() : null, updated_at: new Date().toISOString() }
          : app
      ))
      ...
      console.error('가상 선택 업데이트 오류:', error)
      setError(`가상 선택 업데이트에 실패했습니다: ${error.message}`)
    } finally {
      setProcessing(false)
    }
  }
  ...
  const handleFinalConfirmation = async (applicationId) => {
    try {
      ...
      console.log('최종 확정 시작:', applicationId)

      // 타임아웃 설정
      const timeout = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('최종 확정 타임아웃')), 10000)
      )
      ...
      console.log('최종 확정 완료')
      
      setSuccess(t.finalConfirmSuccess)
      
      // 로컬 상태 업데이트
      setApplications(prev => prev.map(app => 
        app.id === applicationId 
          ? { ...app, status: 'approved', virtual_selected: false, approved_at: new Date().toISOString(), updated_at: new Date().toISOString() }
          : app
      ))
      ...
      setError(`최종 확정에 실패했습니다: ${error.message}`)
    } finally {
      setProcessing(false)
    }
  }
  ...
  const exportToExcel = () => {
    const headers = [t.name, t.age, t.skinType, 'Instagram', 'TikTok', 'YouTube', '상태', t.submittedAt]
    const rows = filteredApplications.map(app => {
      const profile = getUserProfileById(app.user_id)
      return [
        profile?.name || t.noData,
        profile?.age || t.noData,
        profile?.skin_type || t.noData,
        profile?.instagram_url || t.noData,
        profile?.tiktok_url || t.noData,
        profile?.youtube_url || t.noData,
        app.virtual_selected ? t.virtual : (app.status === 'approved' ? t.approved : t.pending),
    ...
    const campaign = filters.campaign !== 'all' ? getCampaignById(parseInt(filters.campaign)) : null
    link.download = `${campaign?.title || 'all_campaigns'}_applications.csv`
    link.click()
  }
  ...
  const filteredApplications = applications.filter(app => {
    ...
    if (filters.status !== 'all') {
      if (filters.status === 'virtual' && !app.virtual_selected) {
        return false
      }
      if (filters.status !== 'virtual' && app.status !== filters.status) {
        return false
      }
    }
  ...
  const stats = {
    ...
    pending: applications.filter(app => app.status === 'pending' && !app.virtual_selected).length,
    approved: applications.filter(app => app.status === 'approved').length,
    rejected: applications.filter(app => app.status === 'rejected').length,
    virtual: applications.filter(app => app.virtual_selected).length
  }
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">{t.virtualSelected}</p>
                  <p className="text-2xl font-bold text-gray-900">{stats.virtual}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600 mb-4">
                        ...
                        <div>
                          <p><strong>나이:</strong> {userProfile.age || t.noData}세</p>
                      ...
                      <div className="flex space-x-4 text-sm">
                        {userProfile.instagram_url && (
                          <a href={userProfile.instagram_url} target="_blank" rel="noopener noreferrer" className="flex items-center text-pink-600 hover:underline">
                            <Instagram className="h-4 w-4 mr-1" />
                            Instagram
                          </a>
                        )}
                        {userProfile.youtube_url && (
                          <a href={userProfile.youtube_url} target="_blank" rel="noopener noreferrer" className="flex items-center text-red-600 hover:underline">
                            <Youtube className="h-4 w-4 mr-1" />
                            YouTube
                          </a>
                        )}
                        {userProfile.tiktok_url && (
                          <a href={userProfile.tiktok_url} target="_blank" rel="noopener noreferrer" className="flex items-center text-black hover:underline">
                      ...
                      return (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p><strong>상태:</strong> {getStatusBadge(selectedApplication.status, selectedApplication.virtual_selected)}</p>
                          </div>
                        </div>
                      )
                    })()}
                      ...
                      return (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <p><strong>{t.age}:</strong> {userProfile.age || t.noData}세</p>
                            <p><strong>{t.skinType}:</strong> {userProfile.skin_type || t.noData}</p>
                          </div>
                          <div>
                ...
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-600">
                    ...
                    {selectedApplication.virtual_selected_at && (
                      <p><strong>가상 선택일:</strong> {formatDate(selectedApplication.virtual_selected_at)}</p>
                    )}
                    {selectedApplication.approved_at && (
                      <p><strong>승인일:</strong> {formatDate(selectedApplication.approved_at)}</p>
                    )}
                  </div>
                  
                  <div className="flex space-x-2">
                    {/* 가상 선택 버튼 */}
                    {selectedApplication.status === 'pending' && !selectedApplication.virtual_selected && (
                      <Button
                        onClick={() => handleVirtualSelection(selectedApplication.id, true)}
                        disabled={processing}
                        className="bg-orange-600 hover:bg-orange-700"
                      >
                        {processing ? (
                    ...
                    {/* 가상 선택된 경우의 버튼들 */}
                    {selectedApplication.virtual_selected && (
                      <>
                        <Button
                          onClick={() => handleFinalConfirmation(selectedApplication.id)}
                          disabled={processing}
                          className="bg-green-600 hover:bg-green-700"
                        >
                          {processing ? (
                        ...
                        <Button
                          variant="outline"
                          onClick={() => handleVirtualSelection(selectedApplication.id, false)}
                          disabled={processing}
                        >
                          {processing ? (
                            <Loader2 className="h-4 w-4 animate-spin mr-2" />
