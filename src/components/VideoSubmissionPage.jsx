          ...
          if (companyProfile.email) {
            await fetch('/.netlify/functions/send-email', {
              ...
              body: JSON.stringify({
                ...
                subject: `[CNEC] ${campaign.title} - 크리에이터 영상 제출 알림`,
                html: `
                  <div style="font-family: 'Noto Sans KR', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                    ...
                    <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
                      ...
                      <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
                        <p style="margin: 0; color: #374151;"><strong>캠페인:</strong> ${campaign.title}</p>
          ...
          <form onSubmit={handleSnsSubmit} className="space-y-4">
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500 mt-1">영상을 SNS에 업로드한 후 URL을 입력해주세요.</p>
              </div>

              <div className="p-4 bg-red-50 border-b border-red-100">
                <p className="text-xs text-red-700 font-medium flex items-center gap-1">
                  <AlertCircle size={12} />
                  영상 검수가 완료된 후 SNS에 업로드해주세요!
                </p>
              </div>
                ...
                {campaign?.ad_code_required && (
                  <div>
                    ...
                    <p className="text-xs text-orange-600 mt-1">
                      인스타그램 업로드 시 파트너십 광고 코드를 입력해주세요.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-3">
              <button
                type="submit"
                disabled={submitting}
                className="flex-1 py-4 bg-purple-600 text-white rounded-2xl font-bold text-base hover:bg-purple-700 disabled:opacity-50 transition-colors"
              >
              ...
              <button
                type="button"
                onClick={() => navigate('/my/applications')}
                className="px-6 py-4 border border-gray-200 rounded-2xl font-bold text-gray-700 hover:bg-gray-50 transition-colors"
              >
                나중에
              </button>
            </div>
