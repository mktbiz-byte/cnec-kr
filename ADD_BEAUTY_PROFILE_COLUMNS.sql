...
-- 피부 고민 (건성 피부, 지성 피부, 복합성 피부, 민감성 피부, 여드름/트러블, 색소침착/기미, 주름/탄력, 모공, 칙칙함/톤, 홍조/혈관, 각질/거칠음)
ALTER TABLE user_profiles
ADD COLUMN IF NOT EXISTS skin_concerns JSONB DEFAULT '[]'::jsonb;

-- 헤어 고민 (건성 머리, 지성 머리, 손상머리, 약한 머리, 비듬/가려움, 두피 지성, 두피 민감, 곱슬머리/프리즈, 매직/펌 손상, 탈색 손상)
ALTER TABLE user_profiles
...
-- 타겟 관심사 (천연/오가닉, 럭셔리, 가성비, 건강/웰니스, 트렌디, 프리미엄)
ALTER TABLE user_profiles
ADD COLUMN IF NOT EXISTS target_interests JSONB DEFAULT '[]'::jsonb;
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_target_gender ON user_profiles(target_gender);
CREATE INDEX IF NOT EXISTS idx_user_profiles_target_age_group ON user_profiles(target_age_group);
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_target_interests ON user_profiles USING GIN (target_interests);
...
COMMENT ON COLUMN user_profiles.target_gender IS '타겟 성별: female, male, all';
COMMENT ON COLUMN user_profiles.target_age_group IS '타겟 연령대: teens, 20s, 30s, 40s_plus, all';
COMMENT ON COLUMN user_profiles.skin_concerns IS '피부 고민 (JSONB 배열): dryness, oiliness, combination_skin, sensitivity, acne, pigmentation, wrinkles, pores, dullness, redness, texture';
COMMENT ON COLUMN user_profiles.hair_concerns IS '헤어 고민 (JSONB 배열): dry_hair, oily_hair, damaged, weak, dandruff, oily_scalp, sensitive_scalp, frizzy, perm_damage, bleach_damage';
...
COMMENT ON COLUMN user_profiles.target_interests IS '타겟 관심사 (JSONB 배열): natural_organic, luxury, value, health_wellness, trendy, premium';
...
ALTER TABLE user_profiles DROP COLUMN IF EXISTS target_gender;
ALTER TABLE user_profiles DROP COLUMN IF EXISTS target_age_group;
...
ALTER TABLE user_profiles DROP COLUMN IF EXISTS target_interests;
*/
